...
Item {
    ...
    required property var buckBarLoader
    ...
    IpcHandler {
        function open(): string {
            if (root.buckBarLoader.item) {
                root.buckBarLoader.item.triggerControlCenterOnFocusedScreen()
    ...
    IpcHandler {
        function open(tab: string): string {
            root.buckDashPopoutLoader.active = true
            if (root.buckDashPopoutLoader.item) {
                switch (tab.toLowerCase()) {
                case "media":
                    root.buckDashPopoutLoader.item.currentTabIndex = 1
                    break
                case "weather":
                    root.buckDashPopoutLoader.item.currentTabIndex = SettingsData.weatherEnabled ? 2 : 0
                    break
                default:
                    root.buckDashPopoutLoader.item.currentTabIndex = 0
                    break
                }
                root.buckDashPopoutLoader.item.setTriggerPosition(Screen.width / 2, Theme.barHeight + Theme.spacingS, 100, "center", Screen)
                root.buckDashPopoutLoader.item.dashVisible = true
        ...
        function close(): string {
            if (root.buckDashPopoutLoader.item) {
                root.buckDashPopoutLoader.item.dashVisible = false
        ...
        function toggle(tab: string): string {
            if (root.buckBarLoader.item && root.buckBarLoader.item.triggerWallpaperBrowserOnFocusedScreen()) {
                if (root.buckDashPopoutLoader.item) {
                    switch (tab.toLowerCase()) {
                    case "media":
                        root.buckDashPopoutLoader.item.currentTabIndex = 1
                        break
                    case "wallpaper":
                        root.buckDashPopoutLoader.item.currentTabIndex = 2
                        break
                    case "weather":
                        root.buckDashPopoutLoader.item.currentTabIndex = SettingsData.weatherEnabled ? 3 : 0
                        break
                    default:
                        root.buckDashPopoutLoader.item.currentTabIndex = 0
                        break
