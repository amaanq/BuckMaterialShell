...
Singleton {
    ...
    Connections {
        ...
        function onNetworkStateUpdate(data) {
            ...
            console.log("DykwabiNetworkService: Subscription update received, networks:", networksCount)
            updateState(data)
        }
    }
    ...
    function updateState(state) {
        ...
        if (vpnIsBusy) {
            ...
            if (busyDuration > timeout) {
                console.warn("DykwabiNetworkService: VPN operation timed out after", timeout, "ms")
                vpnIsBusy = false
                pendingVpnUuid = ""
                vpnBusyStartTime = 0
        ...
        if (pendingConnectionSSID) {
            if (wifiConnected && currentWifiSSID === pendingConnectionSSID && wifiIP) {
                ...
                console.info("DykwabiNetworkService: Successfully connected to", pendingConnectionSSID, "in", elapsed, "ms")
    ...
    function scanWifi() {
        ...
        DykwabiService.sendRequest("network.wifi.scan", null, response => {
            isScanning = false
            if (response.error) {
                console.warn("DykwabiNetworkService: WiFi scan failed:", response.error)
            } else {
    ...
    function submitCredentials(token, secrets, save) {
        console.log("submitCredentials: networkAvailable=" + networkAvailable + " apiVersion=" + DykwabiService.apiVersion)

        if (!networkAvailable || DykwabiService.apiVersion < 7) {
            console.warn("submitCredentials: Aborting - networkAvailable=" + networkAvailable + " apiVersion=" + DykwabiService.apiVersion)
            return
        }
        ...
        DykwabiService.sendRequest("network.credentials.submit", params, response => {
            if (response.error) {
                console.warn("DykwabiNetworkService: Failed to submit credentials:", response.error)
            }
        })
    }
    ...
    function cancelCredentials(token) {
        ...
        DykwabiService.sendRequest("network.credentials.cancel", params, response => {
            if (response.error) {
                console.warn("DykwabiNetworkService: Failed to cancel credentials:", response.error)
            }
        })
    }
    ...
    function refreshVpnProfiles() {
        ...
        DykwabiService.sendRequest("network.vpn.profiles", null, response => {
            if (response.result) {
    ...
    function refreshVpnActive() {
        ...
        DykwabiService.sendRequest("network.vpn.active", null, response => {
            if (response.result) {
    ...
    function connectVpn(uuidOrName, singleActive = false) {
        ...
        DykwabiService.sendRequest("network.vpn.connect", params, response => {
            if (response.error) {
                vpnIsBusy = false
                pendingVpnUuid = ""
                vpnBusyStartTime = 0
    ...
    function disconnectVpn(uuidOrName) {
        ...
        DykwabiService.sendRequest("network.vpn.disconnect", params, response => {
            if (response.error) {
                vpnIsBusy = false
                vpnBusyStartTime = 0
    ...
    function disconnectAllVpns() {
        ...
        DykwabiService.sendRequest("network.vpn.disconnectAll", null, response => {
            if (response.error) {
                vpnIsBusy = false
