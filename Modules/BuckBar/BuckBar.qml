...
import qs.Common
import qs.Modules
import qs.Modules.BuckBar.Widgets
import qs.Modules.BuckBar.Popouts
import qs.Services
import qs.Widgets
...
Item {
    ...
    Variants {
        id: barVariants
        model: SettingsData.getFilteredScreens("buckBar")
        ...
        delegate: PanelWindow {
            ...
            function triggerWallpaperBrowser() {
                buckDashPopoutLoader.active = true
                if (!buckDashPopoutLoader.item) {
                    return
                }

                if (clockButtonRef && clockButtonRef.visualContent && buckDashPopoutLoader.item.setTriggerPosition) {
                    ...
                    buckDashPopoutLoader.item.setTriggerPosition(pos.x, pos.y, pos.width, section, barWindow.screen)
                } else {
                    buckDashPopoutLoader.item.triggerScreen = barWindow.screen
                }

                if (!buckDashPopoutLoader.item.dashVisible) {
                    buckDashPopoutLoader.item.currentTabIndex = 2
                }
                buckDashPopoutLoader.item.dashVisible = !buckDashPopoutLoader.item.dashVisible
            }

            readonly property var dBarLayer: {
                switch (Quickshell.env("DYKWABI_BUCKBAR_LAYER")) {
                case "bottom":
            ...
            property bool gothCornersEnabled: SettingsData.buckBarGothCornersEnabled
            property real wingtipsRadius: Theme.cornerRadius
            readonly property real _wingR: Math.max(0, wingtipsRadius)
            readonly property color _surfaceContainer: Theme.surfaceContainer
            readonly property real _backgroundAlpha: topBarCore?.backgroundTransparency ?? SettingsData.buckBarTransparency
            ...
            readonly property real effectiveBarThickness: Math.max(barWindow.widgetThickness + SettingsData.buckBarInnerPadding + 4, Theme.barHeight - 4 - (8 - SettingsData.buckBarInnerPadding))
            readonly property real widgetThickness: Math.max(20, 26 + SettingsData.buckBarInnerPadding * 0.6)

            screen: modelData
            implicitHeight: !isVertical ? Theme.px(effectiveBarThickness + SettingsData.buckBarSpacing + (SettingsData.buckBarGothCornersEnabled ? _wingR : 0), _dpr) : 0
            implicitWidth: isVertical ? Theme.px(effectiveBarThickness + SettingsData.buckBarSpacing + (SettingsData.buckBarGothCornersEnabled ? _wingR : 0), _dpr) : 0
            color: "transparent"
            ...
            Connections {
                ...
                function onPluginLoaded(pluginId) {
                    console.info("BuckBar: Plugin loaded:", pluginId)
                    SettingsData.widgetDataChanged()
                }
                function onPluginUnloaded(pluginId) {
                    console.info("BuckBar: Plugin unloaded:", pluginId)
                    SettingsData.widgetDataChanged()
                }
            }

            function updateGpuTempConfig() {
                const allWidgets = [...(SettingsData.buckBarLeftWidgets || []), ...(SettingsData.buckBarCenterWidgets || []), ...(SettingsData.buckBarRightWidgets || [])]
            ...
            function createNativeInhibitor() {
                ...
                try {
                    ...
                    nativeInhibitor = Qt.createQmlObject(qmlString, barWindow, "BuckBar.NativeInhibitor")
                    ...
                    nativeInhibitor.enabledChanged.connect(function () {
                        console.log("BuckBar: Native inhibitor enabled changed to:", nativeInhibitor.enabled)
                    ...
                    console.log("BuckBar: Created native Wayland IdleInhibitor for", barWindow.screenName)
                } catch (e) {
                    console.warn("BuckBar: Failed to create native IdleInhibitor:", e)
                    nativeInhibitor = null
                }
            }

            Connections {
                function onBuckBarLeftWidgetsChanged() {
                    barWindow.updateGpuTempConfig()
                }

                function onBuckBarCenterWidgetsChanged() {
                    barWindow.updateGpuTempConfig()
                }

                function onBuckBarRightWidgetsChanged() {
                    barWindow.updateGpuTempConfig()
                }
            ...
            anchors.top: !isVertical ? (SettingsData.buckBarPosition === SettingsData.Position.Top) : true
            anchors.bottom: !isVertical ? (SettingsData.buckBarPosition === SettingsData.Position.Bottom) : true
            anchors.left: !isVertical ? true : (SettingsData.buckBarPosition === SettingsData.Position.Left)
            anchors.right: !isVertical ? true : (SettingsData.buckBarPosition === SettingsData.Position.Right)

            exclusiveZone: (!SettingsData.buckBarVisible || topBarCore.autoHide) ? -1 : (barWindow.effectiveBarThickness + SettingsData.buckBarSpacing + SettingsData.buckBarBottomGap)

            Item {
                id: inputMask

                readonly property int barThickness: Theme.px(barWindow.effectiveBarThickness + SettingsData.buckBarSpacing, barWindow._dpr)

                readonly property bool inOverviewWithShow: CompositorService.isNiri && NiriService.inOverview && SettingsData.buckBarOpenOnOverview
                readonly property bool effectiveVisible: SettingsData.buckBarVisible || inOverviewWithShow
            ...
            Item {
                ...
                property bool reveal: {
                    if (CompositorService.isNiri && NiriService.inOverview) {
                        return SettingsData.buckBarOpenOnOverview || topBarMouseArea.containsMouse || hasActivePopout || revealSticky
                    }
                ...
                Connections {
                    function onBuckBarTransparencyChanged() {
                        topBarCore.backgroundTransparency = SettingsData.buckBarTransparency
                    }
                ...
                MouseArea {
                    ...
                    height: !barWindow.isVertical ? Theme.px(barWindow.effectiveBarThickness + SettingsData.buckBarSpacing, barWindow._dpr) : undefined
                    width: barWindow.isVertical ? Theme.px(barWindow.effectiveBarThickness + SettingsData.buckBarSpacing, barWindow._dpr) : undefined
                    anchors {
                    ...
                    readonly property bool inOverview: CompositorService.isNiri && NiriService.inOverview && SettingsData.buckBarOpenOnOverview
                    hoverEnabled: SettingsData.buckBarAutoHide && !topBarCore.reveal && !inOverview
                    acceptedButtons: Qt.NoButton
                    enabled: SettingsData.buckBarAutoHide && !topBarCore.reveal && !inOverview
                    ...
                    Item {
                        ...
                        transform: Translate {
                            id: topBarSlide
                            x: barWindow.isVertical ? Theme.snap(topBarCore.reveal ? 0 : (SettingsData.buckBarPosition === SettingsData.Position.Right ? barWindow.implicitWidth : -barWindow.implicitWidth), barWindow._dpr) : 0
                            y: !barWindow.isVertical ? Theme.snap(topBarCore.reveal ? 0 : (SettingsData.buckBarPosition === SettingsData.Position.Bottom ? barWindow.implicitHeight : -barWindow.implicitHeight), barWindow._dpr) : 0
                        ...
                        Item {
                            id: barUnitInset
                            anchors.fill: parent
                            anchors.leftMargin: !barWindow.isVertical ? Theme.px(SettingsData.buckBarSpacing, barWindow._dpr) : (axis.edge === "left" ? Theme.px(SettingsData.buckBarSpacing, barWindow._dpr) : 0)
                            anchors.rightMargin: !barWindow.isVertical ? Theme.px(SettingsData.buckBarSpacing, barWindow._dpr) : (axis.edge === "right" ? Theme.px(SettingsData.buckBarSpacing, barWindow._dpr) : 0)
                            anchors.topMargin: barWindow.isVertical ? Theme.px(SettingsData.buckBarSpacing, barWindow._dpr) : (axis.outerVisualEdge() === "bottom" ? 0 : Theme.px(SettingsData.buckBarSpacing, barWindow._dpr))
                            anchors.bottomMargin: barWindow.isVertical ? Theme.px(SettingsData.buckBarSpacing, barWindow._dpr) : (axis.outerVisualEdge() === "bottom" ? Theme.px(SettingsData.buckBarSpacing, barWindow._dpr) : 0)
                            ...
                            Item {
                                id: topBarContent
                                anchors.fill: parent
                                anchors.leftMargin: !barWindow.isVertical ? Math.max(Theme.spacingXS, SettingsData.buckBarInnerPadding * 0.8) : SettingsData.buckBarInnerPadding / 2
                                anchors.rightMargin: !barWindow.isVertical ? Math.max(Theme.spacingXS, SettingsData.buckBarInnerPadding * 0.8) : SettingsData.buckBarInnerPadding / 2
                                anchors.topMargin: !barWindow.isVertical ? 0 : Math.max(Theme.spacingXS, SettingsData.buckBarInnerPadding * 0.8)
                                anchors.bottomMargin: !barWindow.isVertical ? 0 : Math.max(Theme.spacingXS, SettingsData.buckBarInnerPadding * 0.8)
                                clip: false
                                ...
                                Item {
                                    ...
                                    Item {
                                        ...
                                        LeftSection {
                                            ...
                                            axis: axis
                                            widgetsModel: SettingsData.buckBarLeftWidgetsModel
                                            components: topBarContent.allComponents
                                            noBackground: SettingsData.buckBarNoBackground
                                        ...
                                        RightSection {
                                            ...
                                            axis: axis
                                            widgetsModel: SettingsData.buckBarRightWidgetsModel
                                            components: topBarContent.allComponents
                                            noBackground: SettingsData.buckBarNoBackground
                                            parentScreen: barWindow.screen
                                            widgetThickness: barWindow.widgetThickness
                                        ...
                                        CenterSection {
                                            ...
                                            axis: axis
                                            widgetsModel: SettingsData.buckBarCenterWidgetsModel
                                            components: topBarContent.allComponents
                                            noBackground: SettingsData.buckBarNoBackground
                                            parentScreen: barWindow.screen
                                            widgetThickness: barWindow.widgetThickness
                                ...
                                Component {
                                    ...
                                    Clock {
                                        ...
                                        popoutTarget: {
                                            buckDashPopoutLoader.active = true
                                            return buckDashPopoutLoader.item
                                        }
                                        parentScreen: barWindow.screen
                                        ...
                                        onClockClicked: {
                                            ...
                                            if (buckDashPopoutLoader.item) {
                                                buckDashPopoutLoader.item.dashVisible = !buckDashPopoutLoader.item.dashVisible
                                                buckDashPopoutLoader.item.currentTabIndex = 0
                                            }
                                        }
                                    }
                                }
                                ...
                                Component {
                                    ...
                                    Media {
                                        ...
                                        onClicked: {
                                            ...
                                            if (buckDashPopoutLoader.item) {
                                                buckDashPopoutLoader.item.dashVisible = !buckDashPopoutLoader.item.dashVisible
                                                buckDashPopoutLoader.item.currentTabIndex = 1
                                            }
                                        }
                                    }
                                }
                                ...
                                Component {
                                    ...
                                    Weather {
                                        ...
                                        onClicked: {
                                            ...
                                            if (buckDashPopoutLoader.item) {
                                                buckDashPopoutLoader.item.dashVisible = !buckDashPopoutLoader.item.dashVisible
                                                buckDashPopoutLoader.item.currentTabIndex = 3
                                            }
                                        }
                                    }
                                }
                                ...
                                Component {
                                    ...
                                    SystemTrayBar {
                                        ...
                                        parentScreen: barWindow.screen
                                        widgetThickness: barWindow.widgetThickness
                                        isAtBottom: SettingsData.buckBarPosition === SettingsData.Position.Bottom
