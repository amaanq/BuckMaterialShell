...
Item {
    ...
    readonly property real horizontalPadding: SettingsData.buckBarNoBackground ? 2 : Theme.spacingS
    readonly property var hiddenTrayIds: {
        const envValue = Quickshell.env("DYKWABI_HIDE_TRAYIDS") || ""
        ...
        Rectangle {
            ...
            PanelWindow {
                ...
                function updatePosition() {
                    ...
                    const widgetThickness = Math.max(20, 26 + SettingsData.buckBarInnerPadding * 0.6)
                    const effectiveBarThickness = Math.max(widgetThickness + SettingsData.buckBarInnerPadding + 4, Theme.barHeight - 4 - (8 - SettingsData.buckBarInnerPadding))

                    if (menuRoot.isVertical) {
                        const edge = menuRoot.axis?.edge
                        let targetX
                        if (edge === "left") {
                            targetX = effectiveBarThickness + SettingsData.buckBarSpacing + Theme.popupDistance
                        } else {
                            const popupX = effectiveBarThickness + SettingsData.buckBarSpacing + Theme.popupDistance
                        ...
                        if (menuRoot.isAtBottom) {
                            const popupY = effectiveBarThickness + SettingsData.buckBarSpacing + SettingsData.buckBarBottomGap + Theme.popupDistance
                            targetY = screen.height - popupY
                        } else {
                            targetY = effectiveBarThickness + SettingsData.buckBarSpacing + SettingsData.buckBarBottomGap + Theme.popupDistance
                        }
