...
Item {
    ...
    anchors.leftMargin: -(SettingsData.buckBarGothCornersEnabled && axis.isVertical && axis.edge === "right" ? barWindow._wingR : 0)
    anchors.rightMargin: -(SettingsData.buckBarGothCornersEnabled && axis.isVertical && axis.edge === "left" ? barWindow._wingR : 0)
    anchors.topMargin: -(SettingsData.buckBarGothCornersEnabled && !axis.isVertical && axis.edge === "bottom" ? barWindow._wingR : 0)
    anchors.bottomMargin: -(SettingsData.buckBarGothCornersEnabled && !axis.isVertical && axis.edge === "top" ? barWindow._wingR : 0)
    ...
    Canvas {
        ...
        property real wing: SettingsData.buckBarGothCornersEnabled ? Theme.px(barWindow._wingR, dpr) : 0
        property real rt: SettingsData.buckBarSquareCorners ? 0 : Theme.px(Theme.cornerRadius, dpr)
        ...
        onPaint: {
            ...
            const isTop = SettingsData.buckBarPosition === SettingsData.Position.Top
            const isBottom = SettingsData.buckBarPosition === SettingsData.Position.Bottom
            const isLeft = SettingsData.buckBarPosition === SettingsData.Position.Left
            const isRight = SettingsData.buckBarPosition === SettingsData.Position.Right
    ...
    Canvas {
        ...
        property real wing: SettingsData.buckBarGothCornersEnabled ? Theme.px(barWindow._wingR, dpr) : 0
        property real rt: SettingsData.buckBarSquareCorners ? 0 : Theme.px(Theme.cornerRadius, dpr)
        property bool borderEnabled: SettingsData.buckBarBorderEnabled
        ...
        Connections {
            target: SettingsData
            function onBuckBarBorderColorChanged() { root.requestRepaint() }
            function onBuckBarBorderOpacityChanged() { root.requestRepaint() }
            function onBuckBarBorderThicknessChanged() { root.requestRepaint() }
            function onBuckBarSpacingChanged() { root.requestRepaint() }
            function onBuckBarSquareCornersChanged() { root.requestRepaint() }
            function onBuckBarTransparencyChanged() { root.requestRepaint() }
        }
        ...
        onPaint: {
            ...
            const spacing = SettingsData.buckBarSpacing
            ...
            const uiThickness = Math.max(1, SettingsData.buckBarBorderThickness ?? 1)
            const devThickness = Math.max(1, Math.round(Theme.px(uiThickness, dpr)))

            const key = SettingsData.buckBarBorderColor || "surfaceText"
            const base = (key === "surfaceText") ? Theme.surfaceText
                       : (key === "primary") ? Theme.primary
                       : Theme.secondary
            const color = Theme.withAlpha(base, SettingsData.buckBarBorderOpacity ?? 1.0)
