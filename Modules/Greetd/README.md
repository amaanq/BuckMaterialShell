...
sudo ln -sf ~/.cache/quickshell/buckshell/dykwabi-colors.json /var/cache/dykwabi-greeter/colors.json
...
8. (Optional) Install the `dykwabi-greeter-sync` helper for easy theme syncing:
```bash
# Download or copy the dykwabi-greeter-sync script from the spec file
sudo cp /path/to/dykwabi-greeter-sync /usr/local/bin/dykwabi-greeter-sync
sudo chmod +x /usr/local/bin/dykwabi-greeter-sync
```
