...
Item {
    ...
    readonly property bool isRightEdge: SettingsData.buckBarPosition === SettingsData.Position.Right
    ...
    function loadArtwork(url) {
        ...
        if (url.startsWith("http://") || url.startsWith("https://")) {
            const filename = "/tmp/.buckshell/trackart_" + Date.now() + ".jpg"
            activeTrackArtFile = filename

            cleanupProcess.command = ["sh", "-c", "mkdir -p /tmp/.buckshell && find /tmp/.buckshell -name 'trackart_*' ! -name '" + filename.split('/').pop() + "' -delete"]
