...
[![AUR version](https://img.shields.io/aur/version/dykwabi-shell?style=for-the-badge&labelColor=101418&color=9ccbfb)](https://aur.archlinux.org/packages/dykwabi-shell)
...
```bash
# Arch Linux
sudo pacman -S cava wl-clipboard brightnessctl
paru -S matugen-bin dgop stash
...
The example plugin can be installed by `cp -R ./PLUGINS/ExampleEmojiPlugin ~/.config/BuckMaterialShell/plugins` - then it will appear in dykwabi settings.
